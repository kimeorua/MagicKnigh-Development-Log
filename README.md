...
+ ### 06/27 플레이어 행동 및 상태Tag 정의 및 정리 - 기초 -> 언리얼 엔진5.1로 버전업 진행 후 구현함
...
### 체력 회복 구현(05/15)
+ #### R키를 누르면 회복을하는 체력 회복 시스템 구현
+ #### Attribute에 최대 회복 횟수와, 남은 회복 횟수를 추가함
+ #### GA_UsePotion 어빌리티를 만들어, 현재 체력과, 남은 횟수를 조건으로, 사용여부를 확인후, 사용 가능하면, 애니메이션 출력
+ #### AnimState를 통해, 실제 체력이 회복되는 GE_Healing을 호출, 체력을 회복 시키고, 사용횟수를 감소 시킴

### 보스 피격 사운드 및 이펙트 출력, 과부화 수치 자동 감소 구현(05/16)
+ #### 시간에따라 일정 수치 만큼 과부화 수치가 감소하는 시스템을 구현함
---
